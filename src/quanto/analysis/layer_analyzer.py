...
class LayerAnalyzer:
    ...
    def _is_small_dimension(
        self, in_features: int, out_features: int, threshold: int = 4096
    ) -> bool:
        ...
        for name in layer_names:
            ...
            for pattern in gate_patterns:
                if (
                    ...
                    or pattern.replace("*", "").replace("$", "") in name.lower()
                ):
                    gates.append(name)
                    break
        return gates
            ...
            if layer_nums:
                ...
                for i in range(max(0, max_layer - exclude_last_n_layers + 1), max_layer + 1):
                    ...
                    if pattern not in exclude_patterns:
                        exclude_patterns.append(pattern)
                        reasoning[pattern] = (
                            f"Last {exclude_last_n_layers} transformer layers (often sensitive)"
                        )
        else:
    ...
    def get_model_summary(self) -> str:
        ...
        if dims:
            ...
            summary.append("\nDimension Statistics:")
            summary.append(
                f"  Input features: min={min(in_dims)}, max={max(in_dims)}, avg={sum(in_dims) // len(in_dims)}"
            )
            summary.append(
                f"  Output features: min={min(out_dims)}, max={max(out_dims)}, avg={sum(out_dims) // len(out_dims)}"
            )
