    ...
    def _create_samples(self, texts: list[str]) -> list[dict[str, torch.Tensor]]:
        ...
        if current_tokens and len(samples) < self.num_samples:
            ...
            if len(current_tokens) < self.max_length:
                ...
                current_tokens = current_tokens + [pad_id] * (self.max_length - len(current_tokens))
...
class CalibrationDataManager:
    ...
    def load_from_huggingface(self, dataset_name: str = "pileval") -> DataLoader:
        ...
        if dataset_name == "pileval":
            ...
            try:
                ...
                if text_col:
                    texts = [str(t) for t in dataset[text_col] if t and str(t).strip()][
                        : self.num_samples
                    ]
                else:
                    raise ValueError("Could not find text column in dataset")
            except Exception as e:
                raise ValueError(f"Could not load dataset '{dataset_name}': {e}") from e
        ...
        calib_dataloader = DataLoader(
            input_ids, batch_size=self.batch_size, shuffle=False, drop_last=True
        )
    ...
    def load_from_path_or_name(self, data_source: str) -> DataLoader:
        ...
        if data_source in self.SUPPORTED_DATASETS:
