...
class UnifiedQuantizer:
    ...
    def _determine_exclude_layers(self) -> list[str]:
        ...
        # Run iterative sensitivity analysis if enabled
        if self.config.sensitivity_analysis:
            self._log("Running sequential sensitivity analysis...")
            sensitive_layers = self._run_iterative_sensitivity_analysis()
    ...
    def _run_iterative_sensitivity_analysis(self) -> list[str]:
        """
        Run iterative sensitivity analysis for layer exclusion.

        Iteratively:
        1. Run sensitivity analysis to identify layers above threshold
        2. Exclude all layers above threshold
        3. Re-run sensitivity analysis (because excluding layers changes
           activation cache values)
        4. Repeat until: (all layers under threshold) OR
                         (no new exclusions) or
                         (max iterations reached)
        5. Perform final quantization only once after convergence

        Returns:
            Final list of layers to exclude from quantization
        """
        self._log("Running iterative sensitivity analysis...")
        ...
        # Track cumulative excluded layers
        all_excluded: list[str] = []

        # Get initial exclusion list from config
        if self.config.exclude_layers is not None:
            all_excluded.extend(self.config.exclude_layers)
            self._log(f"  Initial excluded from config: {len(self.config.exclude_layers)}")

        self._log(f"  max_iterations: {max_iterations}")
        self._log(f"  threshold: {threshold}")
        self._log(f"  initial cumulative excluded: {len(all_excluded)}")
        ...
        for iteration in range(max_iterations):
            ...
            if not result.success:
                self._log(f"Error in iteration {iteration}: {result.error_message}")
                # Store timing
                if result.timing:
                    self.timing[f"sensitivity_iter_{iteration}"] = result.timing.get("total", 0)
                break
            ...
            # Get new exclusions from this iteration
            new_exclusions = result.excluded_layers

            if not new_exclusions:
                self._log(f"  No new exclusions found in iteration {iteration}")
                break

            # Get scores for layers above threshold
            iteration_scores: dict[str, float] = {}
            for layer_name in new_exclusions:
                if layer_name in result.sensitive_layers:
                    score = result.sensitive_layers[layer_name]
                    if score > threshold:
                        iteration_scores[layer_name] = score
                        all_excluded.append(layer_name)
                        self._log(f"    + {layer_name}: {score:.6f} (above threshold)")
                    else:
                        # Layer under threshold - skip
                        self._log(f"    {layer_name}: {score:.6f} (under threshold - skipping)")
                else:
                    # No score available - add anyway
                    all_excluded.append(layer_name)
                    self._log(f"    {layer_name}: added (no score available)")

            # Check if we found any layers above threshold
            if not iteration_scores:
                self._log("  All layers under threshold. Convergence achieved!")
                break

            # Log summary
            self._log(f"  Added {len(iteration_scores)} new layers to exclude list")
            self._log(f"  Total layers in exclude list: {len(all_excluded)}")
        ...
        self.timing["sensitivity_analysis"] = total_time

        # Print summary
        self._log("\n" + "=" * 60)
        self._log("ITERATIVE SENSITIVITY ANALYSIS SUMMARY")
        self._log("=" * 60)
        self._log(f"Total iterations: {min(iteration + 1, max_iterations)}")
        self._log(f"Total excluded layers: {len(all_excluded)}")

        if all_excluded:
            # Print ASCII table
            self._log("\nExcluded layers:")
            self._log(f"{'Layer':<50} {'Score':<10}")
            self._log("-" * 60)
            for layer in sorted(all_excluded):
                score = scores.get(layer, 0.0)
                self._log(f"{layer:<50} {score:.6f}")
        else:
            self._log("  - no layers excluded -")
