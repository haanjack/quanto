...
class LayerwiseQuantizer:
    ...
    def setup(self) -> None:
        ...
        for template_name in available_templates:
            if (
                self.model_type.lower() in template_name.lower()
                or template_name.lower() in self.model_type.lower()
            ):
    ...
    def load_calibration_data(self) -> torch.Tensor:
        ...
        encoded = self.tokenizer(
            texts, return_tensors="pt", padding=True, truncation=True, max_length=self.seq_len
        )
        ...
        if not weight_files:
            ...
            if index_file.exists():
                with open(index_file) as f:
                    index = json.load(f)
                    weight_files = [
                        model_dir / fname for fname in set(index["weight_map"].values())
                    ]
    ...
    def run_pipeline_quantization(self) -> dict[str, Any]:
        ...
        for layer_idx in tqdm(range(num_layers), desc="Quantizing layers on GPU"):
            ...
            try:
                ...
                self._log("  Quantizing layer weights on GPU...")
                ...
                for sub_name, sub_module in list(layer_module.named_modules()):
                    if isinstance(sub_module, nn.Linear):
                        ...
                        for pattern in self.exclude_layers:
                            if fnmatch.fnmatch(full_name, pattern) or fnmatch.fnmatch(
                                sub_name, pattern
                            ):
                                should_exclude = True
                                break
                ...
                self._log("  Layer quantized on GPU")
            ...
            self._log("  Extracting quantized weights...")
    ...
    def run_layer_offload_quantization(self) -> dict[str, Any]:
        ...
        self._log(
            f"Model loaded to CPU. Total parameters: {sum(p.numel() for p in model.parameters()) / 1e9:.2f}B"
        )
        ...
        for layer_idx in tqdm(range(num_layers), desc="Quantizing layers"):
            ...
            try:
                ...
                self._log("Quantizing layer weights...")
            ...
            # Move layer back to CPU
            layer_module = layer_module.cpu()
            self._log("Layer moved back to CPU")
