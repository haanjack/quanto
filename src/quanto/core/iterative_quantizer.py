...
try:
    ...
    from ..utils.calibration import get_calib_dataloader
except ImportError:
    from analysis.sensitivity_analyzer import (
        SensitivityAnalysisResult,
        SensitivityAnalyzer,
...
class IterativeQuantizer:
    ...
    def evaluate_ppl(self, model: nn.Module | None = None, max_seq_len: int = 2048) -> float:
        ...
        if testenc.input_ids.shape[1] > max_seq_len:
            self._log(
                f"Truncating test data from {testenc.input_ids.shape[1]} to {max_seq_len} tokens"
            )
    ...
    def run(self) -> IterativeResult:
        ...
        try:
            ...
            for iteration in range(1, self.config.max_iterations + 1):
                ...
                if iter_result.ppl_degradation is not None:
                    ...
                    if iter_result.ppl_degradation <= self.config.target_ppl_degradation:
                        self._log(
                            f"Target PPL degradation reached: {iter_result.ppl_degradation:.4f}"
                        )
                        break
            ...
            final_result.best_ppl_degradation = (
                best_ppl_deg if best_ppl_deg != float("inf") else None
            )
    ...
    def _print_final_summary(self, result: IterativeResult) -> None:
        ...
        for iter_result in result.iterations:
            ...
            ppl_str = (
                f"PPL deg: {iter_result.ppl_degradation:.4f}"
                if iter_result.ppl_degradation
                else "N/A"
            )
            print(
                f"  Iteration {iter_result.iteration}: {status}, {ppl_str}, "
