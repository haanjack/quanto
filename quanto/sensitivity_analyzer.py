...
class SensitivityAnalyzer:
    ...
    def _get_base_layer_name(self, quantizer_name: str) -> str:
        ...
        name = quantizer_name
        for suffix in ["._weight_quantizer", "._bias_quantizer",
                       "._input_quantizer", "._output_quantizer"]:
    ...
    def _collect_scale_metrics(self) -> None:
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, QuantMixin):
                for quantizer_attr in ["_weight_quantizer", "_input_quantizer",
                                       "_output_quantizer", "_bias_quantizer"]:
    ...
    def _identify_sensitive_layers(self) -> tuple[list[str], dict[str, str]]:
        ...
        for name, metrics in sorted_layers:
            ...
            if reason is None and metrics.l1_error is not None:
                ...
                if l1_errors:
                    threshold = np.percentile(l1_errors, self.thresholds["l1_error_weight_percentile"])
        ...
        if scale_stats:
            for layer_name, layer_stats in scale_stats.get("scale_stats", {}).items():
                if layer_name in self._metrics:
                    for quant_type in ["_weight_quantizer", "_input_quantizer",
                                       "_output_quantizer", "_bias_quantizer"]:
                        if quant_type in layer_stats:
                            qs = layer_stats[quant_type]
                            self._metrics[layer_name].scale_min = qs.get("scale_min_max", (None, None))[0]
                            self._metrics[layer_name].scale_max = qs.get("scale_min_max", (None, None))[1]
                            self._metrics[layer_name].has_zero_scale = qs.get("has_zero_scale", False)
    ...
    def print_summary(self, result: SensitivityAnalysisResult) -> None:
        ...
        print(f"Layers recommended for exclusion: {result.statistics.get('excluded_count', 0)} "
