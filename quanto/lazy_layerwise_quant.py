...
from int4_pack import quantize_to_int4, pack_int4_to_int32
...
class LazyLayerwiseQuantizer:
    ...
    def get_layer_info(self) -> dict[str, Any]:
        ...
        if "llama" in model_type_lower or "qwen" in model_type_lower:
    ...
    def quantize_layer_weights(
        ...
        quantized_weights = {}
        safe_to_orig = {name.replace(".", "_"): name for name in layer_weights.keys() if len(layer_weights[name].shape) == 2}
    ...
    def _pack_int4_weights(self, weights: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        weight_keys = [k for k in weights.keys()
    ...
    def quantize_batch_layers(
        ...
        class MultiLayerModule(nn.Module):
            ...
            def __init__(self, batch_weights: list[dict[str, torch.Tensor]], layer_indices: list[int]):
                super().__init__()
    ...
    def run_quantization(self) -> dict[str, Any]:
        ...
        self._log(f"\n{'=' * 60}")
        self._log(f"Layer-wise Quantization Pipeline")
        self._log(f"{'=' * 60}")
        ...
        for batch_idx in tqdm(range(total_batches), desc="Quantizing batches"):
            ...
            if not valid_indices:
                self._log(f"  Skipping empty batch")
                continue
            ...
            if len(valid_indices) == 1 or self.batch_size == 1:
                ...
                for i in valid_indices:
                    ...
                    layer_file = Path(self.output_dir) / "quantized_layers" / f"layer_{layer_indices[i]}.safetensors"
                ...
                for result in results:
                    layer_file = Path(self.output_dir) / "quantized_layers" / f"layer_{result['layer_idx']}.safetensors"
                    save_file(result["weights"], layer_file)
                    quantized_layers.append({
            ...
            self._log(f"  Completed in {batch_time:.2f}s ({batch_time/layers_in_batch:.2f}s/layer, ETA: {eta/60:.1f}m)")
        ...
        self._log(f"\n{'=' * 60}")
        self._log(f"QUANTIZATION COMPLETE")
        self._log(f"{'=' * 60}")
...
def main():
    ...
    parser.add_argument("--batch_size", type=int, default=4, help="Number of layers to process in parallel")
    parser.add_argument("--pack_int4", action="store_true", default=True, help="Pack INT4 weights to INT32 (default: True)")
